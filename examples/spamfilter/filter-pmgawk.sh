...
spamly () {
  ...
  $PMGAWK --persist=$HEAPFILE '
    BEGIN { split("", message); k=0; }
    
    { message[k++]=$0; }

    END { if (k==0) {exit;} # empty message or was in the whitelist.

          good_mail_file="good_mail-pmgawk";
          spam_mail_file="spam_mail-pmgawk";
          total_tokens=0.01;
          spamtotal = 0;
          ...
          for (line in message){ 
            ...
            for (i = 0; i <= token_number; i++){
              if (tokens[i] in spam_probability) { 
                if (spam_probability[tokens[i]] <= 0.06 || spam_probability[tokens[i]] >= 0.94){
                  total_tokens+=1;
                  spamtotal+=spam_probability[tokens[i]];
                }
              }
            }
          }
